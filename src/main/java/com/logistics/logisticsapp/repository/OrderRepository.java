...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("""
        ...
        """)
    List<Order> findOrdersByCargoName(
    ...
    @Query(value = """
        ...
        FROM orders o
        JOIN route_vehicle_cargo rvc 
            ON o.id = rvc.order_id

        JOIN cargo c 
            ON rvc.cargo_id = c.id

        LEFT JOIN routes r 
            ON rvc.route_id = r.id

        LEFT JOIN vehicles v 
            ON rvc.vehicle_id = v.id

        WHERE c.name = :cargoName
        """, nativeQuery = true)
    List<Object[]> findOrdersFlat(
        @Param("cargoName") String cargoName
    );
}